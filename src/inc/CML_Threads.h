...
public:
   ...
   /// Check to see if the thread should exit.  Normally this isn't necessary as it's
   /// done automatically when waiting on any semaphore, but if the thread is running
   /// without ever waiting on semaphores then this can be called periodically to ensure
   /// that the system shuts down cleanly
