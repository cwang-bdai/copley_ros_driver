...
public:
   ...
   static const AmpError TryingStopMtr; ///< The amplifier is trying to stop the motor.
...
public:
   ...
   */
   ...
   const Error *GetPositionWrap( uunit &value );
   ...
   */
   ...
   const Error* WriteCCDToFile(const char* name, Network& net);
   ...
   const Error *GetLoadEncoderType( uint16& value );
   const Error *GetMiscellaneousOptions( uint32& value );
   //@}
...
private:
   ...
   bool usingPositionWrap = false;   // Set to true if the encoder closing the p-loop is using position wrap.
   bool positionHasWrapped = false;  // Used by the Amp::SendTrajectory and Amp::PvtStatusUpdate methods to 
                                     // determine if the encoder has wrapped during a PVT stream.
   
   // Value of the position wrap parameter (firmware units) of the encoder that is closing the position loop
   int32 positionWrapValue = 0;
   uunit positionWrapValueUserUnits = 0;
   double numberOfWraps = 0; // = position in user units / wrap value in user units
   double integerValNumberOfWraps = 0; // the integer portion of number of wraps 
   double fractionalValNumberOfWraps = 0; // the fractional portion of number of wraps
   double startingIntValNumWraps = 0; // a copy of the integer portion of number of wraps 
