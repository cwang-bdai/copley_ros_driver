 ...
 */
/***************************************************************************/
const Error *EtherCAT::SetNodeGuard( Node *n, GuardProtocol type, Timeout timeout, uint8 life, Timeout heartbeatPeriod )
{
...
const Error *EtherCAT::SendFrame( EcatFrame *frame, Timeout timeout, int maxRetry )
{
   ...
   do
   {
      ...
      // If timeout was zero, just return without waiting for a response
      if( !timeout ) return 0;

      // Otherwise, wait for response to be received & processed before returning
      Timeout to = timeout;
...
static NodeState FindNodeState( uint ecatState )
{
   switch( ecatState & 0x0F )
   {
      ...
      case 4:  return NODESTATE_SAFE_OP;      // Safe-op state
      case 8:  return NODESTATE_OPERATIONAL;  // Operational state
      default: return NODESTATE_INVALID;
   }
}

static const char *FindNodeStateName( uint ecatState )
{
   return Network::DescribeState( FindNodeState( ecatState ) );
}
...
const Error *EtherCAT::SetNodeAlState( Node *inputNode, uint16 state )
{
   const Error *err;
   uint16 x;
   ...
   cml.Debug( "Attempting to change node %d state to %s\n", inputNode->GetNodeID(), FindNodeStateName( state ) );

   EtherCatNodeInfo *nodeInformation = GetEcatInfo( inputNode );
   if( !nodeInformation ) return &EtherCatError::NodeNotInit;
   
   // Turn off cyclic polling of the node state
   nodeInformation->alStatDG.Disable();

   // Read the current state
   err = NodeRead( inputNode, 0x130, 2, &x );
   if( err ) goto fail;

   if( x & 0x10 )
      cml.Debug( "Starting mode for node %d was 0x%04x (Error %s)\n", inputNode->GetNodeID(), x, FindNodeStateName( x & 0x0F ) );
   else
      cml.Debug( "Starting mode for node %d was 0x%04x (%s)\n", inputNode->GetNodeID(), x, FindNodeStateName( x ) );

   // Clear any pending error first
   if( x & 0x10 )
   {
      cml.Debug( "Writing node %d state value 0x%04x\n", inputNode->GetNodeID(), x );
      err = NodeWrite( inputNode, 0x120, 2, x );
      if( err ) goto fail;

      err = PollChange( inputNode, 0x130, 2, &x );
      if( err ) goto fail;
 
      if( x > 8 )
      {
         uint16 code;
         NodeRead( inputNode, 0x134, 2, &code );
         cml.Error( "Node %d failed to clear state error.  Returned status was 0x%04x, code 0x%04x\n", inputNode->GetNodeID(), x, code );
   ...
   while( 1 )
   {
      inputNode->SetState( FindNodeState(x) );

      // Just return if the mode is correct
      if( x == state )
      {
         nodeInformation->alStatDG.Enable( (uint8) state ); // added explicit casting from CML::uint16 to CML::uint8
         return 0;
      }
      ...
      cml.Debug( "Setting node %d state to %d (%s)\n", inputNode->GetNodeID(), next, FindNodeStateName( next ) );
      err = NodeWrite( inputNode, 0x120, 2, next );
      if( err ) goto fail;
      ...
      if( x & 0x10 )
      {
         NodeRead( inputNode, 0x134, 2, &x );
         cml.Error( "Node %d error changing to state 0x%04x, error code: 0x%04x\n", inputNode->GetNodeID(), state, x );
         return &EtherCatError::NodeStateChange;
      }

      if( x != next )
      {
         cml.Error( "Node %d error changing to state 0x%04x (%s), Returned state was 0x%04x (%s)\n", 
                     inputNode->GetNodeID(), next, FindNodeStateName(next), x, FindNodeStateName(x)  );
         return &EtherCatError::NodeStateChange;
      }
   }

fail:
   cml.Error( "Failed to change node %d to state 0x%04x (%s).  Error: %s\n", inputNode->GetNodeID(), state, FindNodeStateName(state), err->toString() );
   return err;
}

const Error *EtherCAT::SafeOpNode( Node *n )
{
   return SetNodeAlState( n, 4 );
}
...
const Error *EtherCAT::FoE_DnldStart( Node *n, const char *filename, uint32 password, Timeout to )
{
   ...
   if( (int) len > ni->syncLen[SM_RXMBX]-12 )
      return &CanOpenError::BadParam;
   ...
   return FoE_Dnld( n, (uint16) (len+12), to );
}
...
const Error *EtherCAT::FoE_UpldStart( Node *n, const char *filename, uint32 password, Timeout to )
{
   ...
   ni->foePacket = 1;
   const Error *err = FoE_Dnld( n, (uint16) (len+12), to );
   if( err ) return err;
