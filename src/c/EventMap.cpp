...
@param eventMapObj The event map that this event should watch.
...
*/
/***************************************************************************/
const Error *Event::Wait( EventMap &eventMapObj, Timeout timeout )
{
   Semaphore sem;
   ...
   const Error *err = eventMapObj.Add( this );
   if( err )
   {
      mtx.Lock();
      semPtr = 0;
...
@param eventObj Points to the event to add
@return A pointer to an error object on failure, or NULL on success.
*/
/***************************************************************************/
const Error *EventMap::Add( Event *eventObjPntr )
{
   // Make sure the event is available
    if (eventObjPntr->map) {
   ...
   // Add it to my list
   eventObjPntr->map = this;
   eventObjPntr->prev = 0;

   mutex.Lock();
   eventObjPntr->next = list;
   if( list ) list->prev = eventObjPntr;
   list = eventObjPntr;

   // Update the event 
   eventObjPntr->update( mask );
   mutex.Unlock();
...
*/
/***************************************************************************/
const Error *EventMap::Remove( Event *eventObjPntr )
{
   // Make sure I own it
   if( eventObjPntr->map != this ) 
      return &EventError::NotMapped;

   mutex.Lock();

   // Remove it from my list
   if( eventObjPntr->next ) eventObjPntr->next->prev = eventObjPntr->prev;
   if( eventObjPntr->prev ) eventObjPntr->prev->next = eventObjPntr->next;
   else list = eventObjPntr->next;
   mutex.Unlock();
...
void EventMap::update( uint32 newMask )
{
   ...
   for( Event *eventObjPntr = list; eventObjPntr; eventObjPntr = eventObjPntr->next )
      eventObjPntr->update( mask );
}
