...
public:
   ...
   */
   const Error *Init( Amp &amp, uint16 slot, int numAxes )
   {
      ...
      uint16 eventStatusObj = (numAxes>1) ? 0x2185 : OBJID_EVENT_STAT; // numAxes = 1 for all CANopen nodes.
   ...
   */
   void Received( void )
   {
      ...
      for( int i=0; i<axisCt; i++ )
      {
         ...
         if( first[i] || (e!=oldE[i]) || (s!=oldS[i]) || (ins!=oldI) )
         {
...
public:
   ...
   ~PvtThread()
   {
      // Stop my thread before returning so the
      // thread will exit before I destroy the semaphore
      stop();
   }
...
public:
   ...
   void Received( void )
   {
      for( int i=0; i<axisCt; i++ )
      {
         // Only try to process hte PVT data if we have a reference for the
         // axis.  That should always be true for axis[0] but may not be for other
         // axes of a multi-axis drive if the sub-axis hasn't been initialized yet.
         if( ampRef[i] )
...
const Error *Amp::UpdateEvents( uint16 stat, uint32 events, uint16 inputs )
{
   ...
   if( events & ESTAT_STOP        ) mask |= AMPEVENT_TRSTP;
