...
@param timeout The number of milliseconds that the monitor
       thread will wait before indicating an error. Thus, the consumer
       heartbeat interval will be: timeout.
...
const Error *Node::StartHeartbeat( uint16 period, uint16 timeout )
{
   RefObjLocker<Network> net( GetNetworkRef() );
   if( !net ) return &NodeError::NetworkUnavailable;
   return net->SetNodeGuard( this, GUARDTYPE_HEARTBEAT,(Timeout)timeout, 0, (Timeout)period);
}
...
void Node::SetState( NodeState newState )
{
   ...
   if( old != newState )
   {
      cml.Debug( "Node %d state changed from %s to %s\n", GetNodeID(), Network::DescribeState(old), Network::DescribeState(newState) );
