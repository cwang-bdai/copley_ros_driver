...
/***************************************************************************/
/**
  Get the position wrap value. If using a load encoder, its position wrap will be
  returned.
  ...
  */
  /***************************************************************************/
const Error* Amp::GetPositionWrap(uunit& value)
{
    const Error* err = 0;

    uint16 loadEncType = 0;
    GetLoadEncoderType(loadEncType);

    // If the load encoder is not being used, the load encoder type will be 0.
    // If bit 12 is set (new encoding) and bit 14 is set, the load encoder is 
    // in passive mode.
    // If bit 12 is not set (old encoding) and bit 5 is set, the load encoder
    // is in passive mode.
    if ((loadEncType == 0) || ((loadEncType & 0x5000) == 0x5000) || (loadEncType & 0x1020) == 0x20) {
        int motorPositionWrapValueFirmwareUnits = 0;
        err = Upld32(OBJID_MOTOR_WRAP, 0, motorPositionWrapValueFirmwareUnits);
        if (err) { return err; }
        value = PosLoad2User(motorPositionWrapValueFirmwareUnits);
        positionWrapValue = motorPositionWrapValueFirmwareUnits;
        positionWrapValueUserUnits = value;
    }
    else {
        int loadPositionWrapValueFirmwareUnits = 0;
        err = Upld32(OBJID_LOAD_WRAP, 0, loadPositionWrapValueFirmwareUnits);
        if (err) { return err; }
        value = PosLoad2User(loadPositionWrapValueFirmwareUnits);
        positionWrapValue = loadPositionWrapValueFirmwareUnits;
        positionWrapValueUserUnits = value;
    }

    // update the using position wrap private data member variable for this amplifier
    if (value > 0) {
        usingPositionWrap = true;
    }
    else if (value == 0) {
        usingPositionWrap = false;
    }
    else {
...
{
   byte *bytePntr = (byte *)data;

   int32 size = max * 4;

   const Error *err = Upload( OBJID_TRACE_DATA, 0, size, bytePntr );
   if( err ) return err;

   max = size/4;

   for( int i=0; i<max; i++ )
      data[i] = bytes_to_int32( &bytePntr[4*i] );
