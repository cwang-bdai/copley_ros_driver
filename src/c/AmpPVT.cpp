...
const Error *Amp::FormatPvtSeg( int32 pos, int32 vel, uint8 time, uint8 *buff )
{
   ...
   bool posIsRelative = false;

   // If position is more than 24 bits then we will have to send a 
   // relative move segment. I'll just find the difference between 
   // this position and the last one sent.
   // OR if using position wrap, send a relative move segment.
   if(( pos > 0x007FFFFF || -pos > 0x007FFFFF ) || usingPositionWrap)
   {
       // if the position wrapped, we must use a different equation to calculate
       // the relative position
       if (usingPositionWrap && positionHasWrapped) {
           if (pvtLastPos < 0) {
               pos = (-positionWrapValue - pvtLastPos) + pos;
           }
           else {
               pos = (positionWrapValue - pvtLastPos) + pos;
           }
       ...
       // if the result is still more than 24 bits, return an error
   ...
   CML::int32 velLowRes = 0;
   bool velIsLowRes = false;

   // If velocity is more than 24-bits, send it in lower resolution
   if( vel > 0x007FFFFF )
   {
      velLowRes = (vel+50)/100;
      velIsLowRes = true;
   }
   ...
   // set bit 3 of byte 0 to use low-res velocity
   if(velIsLowRes){
       buff[0] |= 0x08;
   }

   // if the low-res velocity is still too large, then we will have
   // to send a relative velocity segment. I'll find the difference
   // between this velocity and the last one sent.
   if ((velIsLowRes) && (velLowRes > 0x007FFFFF || -velLowRes > 0x007FFFFF)){

       CML::int32 velDiff = 0; // in 0.1 counts/sec units
       CML::int32 velDiffLowRes = 0;

       velDiff = vel - pvtLastVel;

       // find the low-res difference in velocity (10 counts/sec units)
       if (velDiff > 0) {
           velDiffLowRes = (velDiff + 50) / 100;
       }
       else if (velDiff < 0) {
           velDiffLowRes = (velDiff - 50) / 100;
       }
       ...
       // if the difference in velocity is still too large, return an error
       if ((velDiffLowRes > 0x007FFFFF) || (-velDiffLowRes > 0x007FFFFF)) {
           return &AmpError::pvtSegVel;
       }

       // if the position was not adjusted to be relative, make the adjustment
       if (!posIsRelative) {
           ...
           // mark the segment as relative
           buff[0] |= 0x10; // bits 3-6 of byte 0 should now form code 7 (bits 3, 4, and 5 are set)
       }
   }
   // if the velocity is low resolution and it falls within the acceptable range, use it
   else if ((velIsLowRes) && !(velLowRes > 0x007FFFFF || -velLowRes > 0x007FFFFF)) {
       vel = velLowRes;
   }
...
const Error *Amp::SendTrajectory( Trajectory &trj, bool start )
{
   ...
   for( i=ct=0; i<n; i++ )
   {
      ...
      // update the number of wraps that have occurred
      if (usingPositionWrap) {
          numberOfWraps = p / positionWrapValueUserUnits;
          fractionalValNumberOfWraps = modf(numberOfWraps, &integerValNumberOfWraps);
          
          // record the initial wrap number
          if (!i) {
              startingIntValNumWraps = integerValNumberOfWraps;
          }

          // if the integer portion of the starting wrap value doesn't equal the integer 
          // portion of the current wrap value, then the encoder has wrapped.
          if (startingIntValNumWraps != integerValNumberOfWraps) {
              startingIntValNumWraps = integerValNumberOfWraps;
              positionHasWrapped = true;
          }
          else {
              positionHasWrapped = false;
          }

          // adjust the position so that it is not greater than one encoder wrap value
          if (AbsoluteValue(numberOfWraps) > 1) {
              p = positionWrapValueUserUnits * fractionalValNumberOfWraps;
          }
      }
      ...
      // if we are using position wrap and it's the first
      // segment of the move, then make sure the initial 
      // position is known.
      if (!i && usingPositionWrap) {
          pvtLastPos = pos;
      }
