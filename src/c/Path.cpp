...
CML_NEW_ERROR( PathError, BadDimensions,  "Path must be two dimensions" );
CML_NEW_ERROR( PathError, BadCenterPoint, "The entered center point results in a radius of zero" );
CML_NEW_ERROR( PathError, NonPositiveRadius, "The radius must be greater than zero" );
...
const Error *Path::AddLine( PointN &p )
{
   ...
   double dx = tempPoint[0] - posEnd[0];
   double dy = 0.0;

   if (GetDim() > 1) {
       dy = tempPoint[1] - posEnd[1];
   }
   ...
   double len = posEnd.distance( tempPoint );
...
const Error *Path::AddArc( PointN &center, double angle )
{
   ...
   if (GetDim() != center.getDim()) {
       CML_ASSERT(GetDim() == center.getDim());
       return &PathError::BadDimensions;
   }
   ...
   // if the radius is 0, use CML_ASSERT and return an error 
   if ((deltaX == 0) && (deltaY == 0)) {
       CML_ASSERT((deltaX != 0) && (deltaY != 0));
       return &PathError::BadCenterPoint;
   }
...
const Error *Path::AddArc( double radius, double angle )
{
   // radius must be a positive value.
    if (radius < 0) {
        CML_ASSERT( radius >= 0 );
        return &PathError::NonPositiveRadius;
    }
